...
class DataBase {
    ...
    function send($sql, $parametros = array()) {
        ...
        var_dump(  $this->consulta = $this->conexion->prepare($sql));
        foreach ($parametros as $nombreParametro => $valorParametro) {
            $this->consulta->bindValue($nombreParametro, $valorParametro);
            var_dump( $this->consulta->bindValue($nombreParametro, $valorParametro));
        }
    ...
    function productoEscalar($proyeccion,$tabla1,$tabla2,$condicion,$parametros=array()){
        $sql="select $proyeccion from $tabla1,$tabla2 where $condicion";
